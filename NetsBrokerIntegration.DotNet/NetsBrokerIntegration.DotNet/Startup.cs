                ...
                Notifications = new OpenIdConnectAuthenticationNotifications
                {
                    ...
                    AuthorizationCodeReceived = async notification =>
                    {
                        using (var client = new HttpClient())
                        {
                            ...
                            if (_signDemo)
                            {
                                var transactionToken = tokenEndpointResult.Value<string>("transaction_token");
                                using (var httpClient = new HttpClient())
                                {
                                    var bearer = HttpContext.Current.Cache["bearer-token"] as string;
                                    var padesRequest = new
                                    {
                                        TransactionTokenJwt = transactionToken
                                    };
                                    var jsonRequest = JsonSerializer.Serialize(padesRequest);

                                    httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", bearer);
                                    var padesResponse = await httpClient.PostAsync(_padesApiUrl, new StringContent(jsonRequest, Encoding.UTF8, "application/json"));
                                    var padesObj = await JsonSerializer.DeserializeAsync<PadesDocument>(await padesResponse.Content.ReadAsStreamAsync());

                                    HttpContext.Current.Session["pades"] = padesObj.padesB64;
                                }
                            }
                        }
                    },
                    ...
                    RedirectToIdentityProvider = async notification =>
                    {
                        ...
                        if (notification.ProtocolMessage.RequestType == OpenIdConnectRequestType.Authentication)
                        {
                            if (_signDemo)
                            {
                                ...
                                //NOTE that this is a barebones demonstration
                                //1: invoke the token endpoint to retrieve the access_token
                                //2: store the access token in a cache near to your service, as this should be reused in its lifetime - avoid invoking the tokenendpoint for each API invocation of signtext api, please reuse access_token
                                //3: invoke signtext api using the access_token as bearer token.
                                ...
                                using (var httpClient = new HttpClient())
                                {
                                    ...
                                    var request = new HttpRequestMessage(HttpMethod.Post, new Uri(_tokenEndpoint))
                                    {
                                        Content = formContent
                                    };
                                    ...
                                    {
                                        var content = await JsonSerializer.DeserializeAsync<TokenEndpointResponse>(await response.Content.ReadAsStreamAsync());
                                        var access_token = content.Access_token;

                                        HttpContext.Current.Cache["bearer-token"] = access_token;

                                        var signtextApiRequest = new
                                        {
                                            signTextBase64 = _testPdf,
                                            signTextType = "pdf"
                                        };
                                        var jsonRequest = JsonSerializer.Serialize(signtextApiRequest);

                                        httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", access_token);
                                        var signtextIdResponse = await httpClient.PostAsync(_signtextApiUrl, new StringContent(jsonRequest, Encoding.UTF8, "application/json"));
                                        var signtextidContent = await JsonSerializer.DeserializeAsync<SignTextApiResponse>(await signtextIdResponse.Content.ReadAsStreamAsync());
                                        _promptMode = "login";
                                        notification.ProtocolMessage.Parameters.Add("signtext_id", signtextidContent.signTextId);
                                        notification.ProtocolMessage.Parameters["scope"] = notification.ProtocolMessage.Parameters["scope"] + " transaction_token";
                                    }

                                }
                            }

                            if (_promptMode != null && !notification.ProtocolMessage.Parameters.ContainsKey("prompt"))
                            {
